...
| Data minimisation at write time | Identifiers are detected and tokenised before anything reaches disk. Query audio is held in memory only for as long as transcription needs it; call recordings are a separate consent-gated encrypted store (D21). Each trace stage stores the SHA-256 digest of its input, never the input. | `backend/app/security/pii.py`, `trace._input_digest`, `turn.finalise` | `test_scenarios.py::test_beat_8_pii_is_tokenised_and_the_turn_path_writes_no_plaintext_audio`, `test_persistence.py::test_no_unencrypted_audio_on_disk`, `test_persistence.py::test_audio_is_not_stored_without_consent` | *to fill* |
...
| Auditing the auditors | Every compliance search, every call opened and every recording played is written to `access_log` and appended to the ledger as a `compliance_access` record. Only `compliance_officer` and `admin` hold the permissions, and recording playback re-checks the plaintext digest before it returns audio. | `backend/app/compliance.py record_access`, `backend/app/security/recordings.py read`, `rbac.PERMISSIONS` | `test_compliance_lookup.py::test_every_look_is_written_to_the_access_log_and_the_ledger`, `test_compliance_lookup.py::test_a_customer_token_cannot_reach_any_of_the_lookup`, `e2e/test_dashboard.py::test_an_officer_plays_a_recording_and_the_play_is_logged` | *to fill* |
