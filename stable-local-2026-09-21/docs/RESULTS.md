...
Generated: 2026-09-20T18:18:24.455411+00:00
...
| Metric | Value | Note |
|---|---|---|
| Accuracy on the eval set | 0.8889 | 54 utterances, held out from the head's training bank |
| 5-fold CV accuracy on the seed bank | 0.8359493670886076 | 397 seeded utterances, std 0.0663119602903705 |

Top confusions: `product_info -> fraud_report` x2, `product_info -> dispute_txn` x1, `product_info -> limit_change` x1, `out_of_scope -> get_balance` x1, `out_of_scope -> branch_ifsc` x1
...
| Mean top score, answered from core banking | 0.4242 | 28 account and transaction queries, excluded from the calibration: they are neither grounding positives nor must-refuse negatives |
...
| Metric | Value | Note |
|---|---|---|
| WER | 0.4975 | 54 utterances, 402 reference words, model faster-whisper/base/int8 |
| Substitutions / deletions / insertions | 158 / 20 / 22 |  |
|   WER, en | 0.0944 | 28 utterances, 233 reference words |
|   WER, hi | 1.05 | 10 utterances, 60 reference words |
|   WER, mix | 1.0143 | 8 utterances, 70 reference words |
...
| ASR latency p50 / p95 | 1045.9 ms / 4823.2 ms | CPU, int8 |

## Speaker verification and anti-spoofing (A6)

| Metric | Value | Note |
|---|---|---|
| Speaker EER | 0.0 | 6 target and 12 non-target trials over 3 speakers, model speechbrain/spkrec-ecapa-voxceleb@cosine. Trials: held-out clips, phrases the enrolment never saw. |
| theta at EER | 0.7956 | written to runtime/calibration.json and used by the live system |

> An equal error rate of zero is not a strong result here, it is a consequence of the trial set. Three distinct text to speech voices are trivially separable by any speaker embedding, and the trial counts above are small. Read this as evidence that the verification path is wired up and scoring correctly, not as a measure of how it would perform on real speakers over a telephone line.
| Countermeasure EER | 0.0 | 6 bona fide and 9 spoof clips, LFCC+GMM baseline (BASELINE, not AASIST) |
| min t-DCF (normalised) | undefined at this operating point | the speaker verification system already rejects every spoof at its own threshold, so the countermeasure carries no tandem cost. C1 = 0.9405, C2 = 0.0, priors {'pi_target': 0.9405, 'pi_nontarget': 0.009499999999999995, 'pi_spoof': 0.05}, costs {'C_miss': 1.0, 'C_fa': 10.0} |
...
From 139 recorded traces. The p95 for the `nlu` stage includes the first turn after a cold start, which pays the one-off embedding model load; the server warms that at startup so a live demo does not see it.

| Stage | n | p50 ms | p95 ms |
|---|---|---|---|
| **turn total** | 139 | 32.42 | 116.69 |
| anomaly | 117 | 0.2 | 0.36 |
| fusion | 117 | 0.18 | 0.29 |
| langid | 139 | 0.23 | 0.5 |
| nlu | 117 | 29.18 | 111.35 |
| otp | 16 | 0.05 | 0.08 |
| pii_redaction | 139 | 1.18 | 2.8 |
| policy | 117 | 0.08 | 0.12 |
| readback | 6 | 0.04 | 0.07 |
| retrieval | 25 | 25.12 | 34.2 |
| risk | 117 | 0.09 | 0.19 |
| speaker_verify | 92 | 0.14 | 0.38 |
| text_input | 139 | 0.06 | 0.11 |
